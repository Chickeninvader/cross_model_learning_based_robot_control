#!/bin/bash
#SBATCH -N 1                    # number of nodes
#SBATCH -c 16                   # number of cores
#SBATCH -t 3-00:00:00           # time in d-hh:mm:ss
#SBATCH -p general              # partition
#SBATCH -q public               # QOS
#SBATCH --gres=gpu:a100:2       # GPU allocation
#SBATCH --mem=128G              # memory
#SBATCH -o slurm.%j.out         # file to save job's STDOUT (%j = JobId)
#SBATCH -e slurm.%j.err         # file to save job's STDERR (%j = JobId)
#SBATCH --mail-type=ALL         # Send an e-mail when a job starts, stops, or fails
#SBATCH --mail-user=kpham34@asu.edu
#SBATCH --export=NONE           # Purge the job-submitting shell environment
#SBATCH --job-name=robomimic_train

echo "=========================================="
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $(hostname)"
echo "Started: $(date)"
echo "=========================================="

# Load mamba module
module load mamba/latest

# Activate robomimic environment
source activate robomimic

# Verify environment
echo "Python: $(which python)"
python --version
echo "PyTorch version:"
python -c "import torch; print(torch.__version__); print('CUDA available:', torch.cuda.is_available()); print('CUDA devices:', torch.cuda.device_count())"
echo "Environment: $CONDA_DEFAULT_ENV"

# Verify GPU setup
echo "=========================================="
echo "GPU Information:"
nvidia-smi --query-gpu=index,name,memory.total,memory.free --format=csv
echo "=========================================="

# Navigate to working directory
cd /scratch/kpham34/robomimic || exit

# Set environment variables
export CUDA_VISIBLE_DEVICES=0,1
export PYTHONPATH="${PWD}:${PYTHONPATH}"
export OMP_NUM_THREADS=8

# Training configuration
DATASET=/scratch/kpham34/data/robosuite/lift.hdf5
CONFIG=configs/bc_transformer.json
SAVE_DIR=./checkpoints/bc_transformer

echo "Training Configuration:"
echo "  Dataset: $DATASET"
echo "  Config: $CONFIG"
echo "  Save dir: $SAVE_DIR"
echo "  GPUs: 2"
echo "  Working dir: $PWD"
echo "=========================================="

# Run behavior cloning training
python scripts/train.py \
  --config=$CONFIG \
  --dataset=$DATASET \
  --output_dir=$SAVE_DIR \
  --gpu_ids=0,1 \
  --num_epochs=500

echo "=========================================="
echo "Training completed: $(date)"
echo "Final GPU status:"
nvidia-smi --query-gpu=index,memory.used --format=csv
echo "=========================================="
