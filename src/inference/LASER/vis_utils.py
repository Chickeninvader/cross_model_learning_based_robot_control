...
def get_mask_one_image(frame_image, masks=None, oid_class_pred=None, frame_id=None, manual_masks=None):
    """
    Display masks on a single frame image.

    Parameters:
    - frame_image: HxWxC image array
    - masks: dict or list of masks (optional)
    - oid_class_pred: dict of class names per object id (optional)
    - frame_id: integer frame index (used to look up manual_masks)
    - manual_masks: optional dict {frame_id: {obj_id: mask_array}} to prefer over `masks`

    Returns: (fig, ax)
    """
    # If no masks provided, but manual_masks has an entry for this frame, use it
    if (masks is None or len(masks) == 0) and manual_masks is not None and frame_id is not None:
        masks = manual_masks.get(frame_id, None)
    ...
    if masks is None:
        # nothing to show, just return the figure
        plt.show()
        return fig, ax
    ...
    # If masks is a torch Tensor with shape (N, H, W) or (H, W, N), convert to dict
    if isinstance(masks, torch.Tensor):
        # assume (N, H, W)
        if masks.ndim == 3:
            masks = {i: masks[i].cpu().numpy() for i in range(masks.shape[0])}

    # Add the masks
    for obj_id, mask in masks.items():
        det_class = None
        # Normalize object id to int when possible
        try:
            obj_key = int(obj_id)
        except Exception:
            obj_key = obj_id

        # Resolve class name from various possible containers (dict, list, tuple)
        if oid_class_pred is not None:
            try:
                if isinstance(oid_class_pred, dict):
                    det_name = oid_class_pred.get(obj_key, oid_class_pred.get(obj_id, None))
                elif isinstance(oid_class_pred, (list, tuple)):
                    det_name = None
                    if isinstance(obj_key, int) and 0 <= obj_key < len(oid_class_pred):
                        det_name = oid_class_pred[obj_key]
                else:
                    det_name = None
                if det_name is not None:
                    det_class = f"{obj_id}. {det_name}"
            except Exception:
                det_class = None

        # Fallback to showing object id only when no class prediction available
        if det_class is None:
            det_class = f"Obj {obj_id}"

        show_mask(mask, ax, obj_id=obj_id, det_class=det_class, random_color=False)

    # Ensure display in notebooks and GUIs
    plt.show()
    return fig, ax
